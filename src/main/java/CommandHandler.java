public class CommandHandler {
    private final TaskManager taskManager;
    private final Ui ui;

    public CommandHandler(TaskManager taskManager, Ui ui) {
        this.taskManager = taskManager;
        this.ui = ui;
    }

    public boolean processCommand(String userInput) {
        String[] words = userInput.split(" ", 2);
        String command = words[0].toLowerCase();
        String description = (words.length > 1) ? words[1] : "";

        switch (command) {
        case "bye":
            return false;
        case "list":
            ui.showTaskList(taskManager.getTasks());
            break;
        case "mark":
            handleMark(true, words);
            break;
        case "unmark":
            handleMark(false, words);
            break;
        case "todo":
            addTodo(description);
            break;
        case "deadline":
            addDeadline(description);
            break;
        case "event":
            addEvent(description);
            break;
        default:
            ui.showUnknownCommand();
        }
        return true;
    }

    private void addTodo(String description) {
        if (description.isBlank()) {
            ui.showEmptyDescription("todo");
            return;
        }
        Task task = new Todo(description);
        taskManager.addTask(task);
        ui.showAdded(task, taskManager.getTaskCount());
    }

    private void addDeadline(String description) {
        String[] parts = description.split("/by", 2);
        if (parts.length < 2) {
            ui.showInvalidFormat("deadline", "<desc> /by <time>");
            return;
        }
        Task task = new Deadline(parts[0].trim(), parts[1].trim());
        taskManager.addTask(task);
        ui.showAdded(task, taskManager.getTaskCount());
    }

    private void addEvent(String description) {
        String[] parts = description.split("/from", 2);
        if (parts.length < 2) {
            ui.showInvalidFormat("event", "<desc> /from <start> /to <end>");
            return;
        }
        String[] fromTo = parts[1].split("/to", 2);
        if (fromTo.length < 2) {
            ui.showInvalidFormat("event", "<desc> /from <start> /to <end>");
            return;
        }
        Task task = new Event(parts[0].trim(), fromTo[0].trim(), fromTo[1].trim());
        taskManager.addTask(task);
        ui.showAdded(task, taskManager.getTaskCount());
    }

    private void handleMark(boolean isMark, String[] words) {
        if (words.length < 2) {
            ui.showMissingTaskNumber(isMark);
            return;
        }
        try {
            int taskNumber = Integer.parseInt(words[1]);
            if (taskManager.isInvalidTaskNumber(taskNumber)) {
                ui.showInvalidTaskNumber();
                return;
            }
            Task task = taskManager.getTask(taskNumber);
            if (isMark) {
                task.markAsDone();
            } else {
                task.markAsNotDone();
            }
            ui.showMark(task, isMark);
        } catch (NumberFormatException e) {
            ui.showInvalidNumberFormat();
        }
    }
}
